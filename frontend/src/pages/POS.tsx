...
export default function POS() {
  ...
  useEffect(() => {
    // Try fetching from backend, fallback to static data
    getMenu().then(items => {
      if (items.length > 0) {
        setMenu(items)
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      try {
        ...
        showToast(`Bill #${order.invoiceNo} printed & saved!`)
      } catch {
        showToast(`Bill #${order.invoiceNo} saved! (Print failed - check printer)`)
      }
      setCart([])
    } catch {
      ...
      try {
        ...
        showToast(`Bill #${invoiceNo} printed & saved locally!`)
      } catch {
        showToast(`Bill #${invoiceNo} saved locally! (Printer not connected)`)
      }
      setCart([])
    }
    setLoading(false)
              ...
              <div key={item.name} className="bg-gray-800 rounded-lg p-2 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => updateQty(item.name, -1)}
                    className="w-7 h-7 rounded-full bg-red-600/20 text-red-400 font-bold text-sm flex items-center justify-center hover:bg-red-600/40">−</button>
                  <span className="w-5 text-center text-sm font-bold">{item.quantity}</span>
                  <button onClick={() => updateQty(item.name, 1)}
                    className="w-7 h-7 rounded-full bg-green-600/20 text-green-400 font-bold text-sm flex items-center justify-center hover:bg-green-600/40">+</button>
          ...
          <div className="border-t border-gray-700 p-3 bg-gray-800 space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPaymentMethod('cash')}
                className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${paymentMethod === 'cash' ? 'bg-green-600' : 'bg-gray-700 hover:bg-gray-600'}`}>
                💵 Cash
              </button>
              <button onClick={() => setPaymentMethod('digital')}
                className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${paymentMethod === 'digital' ? 'bg-blue-600' : 'bg-gray-700 hover:bg-gray-600'}`}>
                📱 Digital
              </button>
            </div>
